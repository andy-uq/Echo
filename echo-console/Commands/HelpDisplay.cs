...
");
	...
	partial class Command
	{
	    public void Help()
		{
			...
			{
				System.Console.Write("{0} ", name.ToUpper());
			    foreach (IArgument arg in Arguments)
                {
					...
					if (arg.IsAllArguments)
					{
						System.Console.Write("[argument1] [argument2] [etc]"); 
						continue;
					}

					System.Console.Write("{0} ", arg.FormatSyntax(arg.DisplayName));
				}
			...
			System.Console.WriteLine();
			System.Console.WriteLine((string) this.Description);
			System.Console.WriteLine();
	...
	partial class Argument
	{
		public string FormatSyntax(string name)
		{
			...
			sb.Append('/');
			sb.Append(name);
            if (!IsFlag) sb.AppendFormat((string) "=]{0}", (object) UnderlyingType.Name);
			if (!Required) sb.Append(']');
			return sb.ToString();
		}
		...
		public void Help()
		{
			System.Console.Write("  {0}", FormatSyntax(DisplayName));

            if (!Required && !IsFlag && DefaultValue != null)
				System.Console.Write((string) " ({0})", (object) this.DefaultValue);

			List<string> alt = new List<string>(AllNames);
			alt.Remove(DisplayName);
			if( alt.Count > 0 )
				System.Console.Write(" [/{0}{1}]", String.Join("=|/", alt.ToArray()), IsFlag ? String.Empty : "=");

			System.Console.Write((string) " {0}", (object) this.Description);
			System.Console.WriteLine();
		}
	}
	...
	partial class Option
	{
		public void Help()
		{
			...
			foreach (string name in this.AllNames)
			{
				System.Console.WriteLine("GET {0}", name.ToUpper());
				System.Console.WriteLine("SET {0} [value]", name.ToUpper());
			}
